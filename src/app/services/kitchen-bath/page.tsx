...
export default function Page() { return (<>
  <section className="relative h-[50vh] min-h-[400px] flex items-end overflow-hidden">
    ...
    <div className="relative z-10 max-w-7xl mx-auto w-full px-6 md:px-12 pb-12">
      ...
      <h1 className="font-head text-[clamp(3rem,7vw,6rem)] leading-none">Kitchen & <span className="text-[#E87B2E]">Bath.</span></h1></AnimateIn>
    </div></section>
  <section className="py-24 md:py-36 px-6 md:px-12"><div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16">
    ...
    <Link href="/contact" className="inline-block mt-8 bg-[#E87B2E] text-black font-cond font-bold text-sm tracking-[2px] uppercase px-8 py-4 hover:bg-[#F28C3A] transition-colors">Get A Free Quote →</Link></AnimateIn></div>
    <AnimateIn delay={0.2}><div className="grid grid-cols-2 gap-3">
      {svc.gallery.map((img,i)=>(<div key={i} className={`relative overflow-hidden ${i===0?'col-span-2 h-64':'h-48'}`}><Image src={img} alt={`${svc.title} project ${i+1}`} fill className="object-cover hover:scale-105 transition-transform duration-700" /></div>))}
